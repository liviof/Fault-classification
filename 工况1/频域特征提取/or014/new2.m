...
% IMF6 = zeros(size(last_data));
% IMF7 = zeros(size(last_data));
...
for i = 1:size(new_data, 2)
    ...
    % 对样本进行VMD处理
    K= 5;
    [u, residual, info] = vmd(sample, 'NumIMFs',5); % 假设参数值
...
% ------- 2. 能量占比（所有分量） -------
modal_energies = squeeze(sum(vmd_components.^2, 2)); % 每个分量在所有行上累加能量 (5 x 1024)
total_energy = sum(modal_energies, 'all');            % 所有分量总能量
energy_ratio = sum(modal_energies, 2) / total_energy; % 每个分量的能量占比

% 可视化能量占比（所有分量）
figure;
bar(1:num_components, energy_ratio);
title('K=7时所有分量的能量占比');
xticklabels(compose('IMF %d', 1:num_components));
xlabel('模态分量编号');
ylabel('能量占比');
...
% ------- 3. 频谱峭度（所有分量） -------
spectral_kurt = zeros(num_components, 1);
for i = 1:num_components
    component = squeeze(vmd_components(i, :, :)); % 行：样本；列：时间点
    component_mean = mean(component, 1);           % 平均成一个序列
    fft_comp = abs(fft(component_mean));
    power_spectrum = fft_comp.^2;
    spectral_kurt(i) = kurtosis(power_spectrum);
end

% 可视化频谱峭度（所有分量）
figure;
bar(1:num_components, spectral_kurt);
title('所有分量的频谱峭度');
xticklabels(compose('IMF %d', 1:num_components));
xlabel('模态分量编号');
ylabel('频谱峭度值');
...
fprintf('\n【当前K值下，所有分量评估结果】：\n');
for i = 1:num_components
    fprintf('IMF %d → 相关系数：%.4f | 能量占比：%.4f | 频谱峭度：%.4f\n', ...
        i, correlation_coeffs(i), energy_ratio(i), spectral_kurt(i));
end
