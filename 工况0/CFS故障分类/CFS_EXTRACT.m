...
%% 重要的参数icl cl ；gammadmeo按照从大到小的顺序排列；inddemo 存储的是排序前 gamma 中各元素在原数组中的索引
...
% 归一化
[normalizedData, settings] = mapminmax(CFS_Data,0,1);
% [normalizedData, settings] = mapminmax(X,0,1);

data = normalizedData;%——————————————————要求分类情况，不转置
...
title ('LMD-SVD-CFS-bear14','FontSize',16,'fontname','Times New Roman')
xlabel ('The value of \rho','FontSize',16,'fontname','Times New Roman');
ylabel ('The value of \delta','FontSize',16,'fontname','Times New Roman');
set(gca,'FontSize',16);
...
% 绘制聚类中心并用蓝色框选出来
center_box_width = 0.05; % 框宽度的一半（根据需要调整）
center_box_height = 0.05; % 框高度的一半（根据需要调整）
...
for i = 1:NCLUST
    legend_str{i} = sprintf('Cluster %d', i);
end
legend(legend_str);


text(rho(icl(i)),delta(icl(i)),['NR-',num2str(icl(i))],'FontSize',10,'fontname','Times New Roman')
title('Clusters Visualization','FontSize',16,'fontname','Times New Roman');
ylabel('The value of \gamma','FontSize',16);
xlabel('The number of sample','FontSize',16);
set(gca,'FontSize',16);
